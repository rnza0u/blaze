...
import { useLatestVersion } from '@site/src/hooks/latest-version'
import CodeBlock from '@theme/CodeBlock'
import Admonition from '@theme/Admonition'
import BrowserOnly from '@docusaurus/BrowserOnly'
import { packageDownloadUrl } from '@site/src/services/util'
...
- <BrowserOnly>{() => <a target="_blank" href={packageDownloadUrl('latest', 'x86_64-windows').toString()}>Download the latest package for Windows.</a>}</BrowserOnly>
...
<BrowserOnly>{
  () => {
    const InstallFromSourceCodeInstructions = () => {
      ...
      return (
        <>
          ...
          <CodeBlock language='sh'> 
            {
              [
                `git clone --depth 1 --branch ${latestVersion ?? '<version>'} https://github.com/rnza0u/blaze.git`,
                './blaze/bootstrap.sh'
              ].join('\n')
            }
          </CodeBlock>
    ...
    return <InstallFromSourceCodeInstructions />
  }
}</BrowserOnly>
