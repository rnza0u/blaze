...
`ForceExit` might not work as expected when using the `shell` option. Its behavior is to force-exit parent processes, not whole process groups.

For complex use cases where you need to kill detached shell processes, the [`std:exec`](./exec.mdx) would be more convenient.
